...
<font size="7">Go to <a href="http://bit.ly/msfr24_materials">http://bit.ly/msfr2025-materials</a> and download <b>2-tidy_clean.pdf</b></font>
...
<html><div style='float:left'></div><hr color='#FFFFFF' size=1px width=1100px></html>
...
<html><div style='float:left'></div><hr color='#9a2598' size=1px width=1100px></html>
...
knitr::include_graphics("img/session2/tidy_data.png")
```

- Every other arrangement of data is untidy
...
- Level of the attributes: **village** (one row = one village)
...
R has different packages to read different types of data. Today we'll work with the following:

1. `haven` : Reads and writes data formats used by other statistical software (e.g., `.dta` for Stata)

2. `readxl` : Reads Excel `.xls` and `.xlsx` files into R
...
1. `dplyr`: Core package for manipulating dataframes (e.g., filtering, summarizing, grouping)

2. `janitor`: Helps with basic cleaning tasks like fixing column names and removing empty rows

3. `labelled`: Manages variable labels, value labels, and metadata
...
- `n_distinct()` is now counting the number of unique combinations in the subset dataframe resulting from `select()` (as opposed to counting the unique observations in the full data `df_hh`, which is what we did in exercise 3)
...
```{r}
df_hh_dedup <- df_hh %>% 
  distinct(vid, hhid, .keep_all = TRUE)
...
- Real-world data is often **not tidy**
  ...
  - Wide formats that hide structure

#### Original (Wide) Format

```{r echo = FALSE, out.width="70%"}
knitr::include_graphics("img/session2/wide.png")
```

- Harder to **analyze**, **filter**, or **plot** by site or species.
- Not tidy: site names are **column headers**, not values.
...
### ✅ After Reshape: Long Format with `pivot_longer()`
...
.pull-right4[
```{r echo = FALSE, out.width="60%"}
knitr::include_graphics("img/session2/long.png")
```
]

.pull-left4[
- Easier for:
  - 🧪 filtering (e.g. only *Quercus*)
  - 📈 plotting (e.g. DBH by site)
  - 📊 modeling (grouped summaries, regression)
]  
---
