...
.pull-left[
1. Go to this page: https://osf.io/7rw5v/files/ and download the file in `Data` >> `admin_data.csv`
...
- Subset the dataframe again and save the result into a new dataframe called `admin_data_kayonza`. You can use this code for that:

  ```{r eval=FALSE}
  admin_data_kayonza <- subset(admin_data, District == "KAYONZA")
  ```

- Use `View(admin_data_kayonza)` to visualize the result (notice the uppercase "V")
...
1. Install the library `here` by using `install.packages()`
...
Now `here` is installed, we only need to load them to start using the functions they have.
...
1. Go to https://osf.io/7rw5v/files/ and download the file: `Data` >> `DataWork.zip`
...
Pipes are part of the libraries included in the tidyverse. When you loaded `dplyr` in exercise 9, the pipes become available for use
...
```{r eval=FALSE}
admin_data_long <- here("DataWork", "data", "raw", "admin_data_long.csv") %>% read.csv()
```
...
```{r echo = FALSE, out.width="70%"}
knitr::include_graphics("img/session1/env-admin-data-long.png")
```
