...
SHOW TABLE AFTER SOLVING RENDERING ISSUE
...
quick_xlsx(
  descriptives_income,
  file = here("DataWork", "outputs", "descriptives_income.xlsx")
)
```
...
quick_xlsx(
  desc_income,
  file = here("Data", "DataWork", "outputs", "descriptives_income.xlsx")
)
```
...
quick_xlsx(
  desc_income,
  file = here("DataWork", "outputs", "descriptives_income_formatted.xlsx")
)
...
quick_xlsx(
  desc_income,
  file = here("Data", "DataWork", "outputs", "descriptives_income_formatted.xlsx")
)
```
...
1\. Estimate three regressions of `livestock_now` on (1) `hh_size` only; (2) `hh_size` and `livestock_before`; (3) `hh_size`, `livestock_before`, and `energy` (note that this last variable is a facotr). Let's not focus on the correctness of proposing these models for now.
